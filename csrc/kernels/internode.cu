...
dispatch(int4* recv_x, float* recv_x_scales, int64_t* recv_topk_idx, float* recv_topk_weights, SourceMeta* recv_src_meta,
    ...
    auto rdma_channel_bitmap = SymBuffer<uint64_t, false>(rdma_buffer_ptr, 1, kNumRDMARanks, channel_id, num_channels);
    ...
    if (warp_role == WarpRole::kRDMASender) {
        ...
        int new_rdma_channel_bitmap[128];
        memset(new_rdma_channel_bitmap, 0, sizeof(new_rdma_channel_bitmap));
